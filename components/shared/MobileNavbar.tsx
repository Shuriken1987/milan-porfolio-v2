...
export const MobileNavbar: React.FC<MobileNavbarProps> = ({ navLinks }) => {
  ...
  return (
    ...
    >
      ...
      >
        <motion.button
          className="self-end outline-none border-none bg-transparent uppercase text-white font-[13px] cursor-pointer mt-[30px] mr-[25px] mb-0 ml-0"
        ...
        <motion.ul className="list-none mt-[40px]" variants={ulVariant}>
          {navLinks.map((navItem) => (
            <motion.li
              whileTap={{ scale: 0.95 }}
              key={navItem.id}
              className="mt-[20px] mb-0 overflow-y-hidden select-none cursor-pointer"
            >
              <motion.div
                className={`text-white font-grandSlang font-extrabold text-[34px] mb-10    
